...
function SubmitEADForm(sender, e) {
    ...
    //Create the UserReview element if not found
    if ($('#UserReview').length == 0) {
        var newElement = document.createElement("input");
        newElement.type = "hidden";
        newElement.id = 'UserReview';
        newElement.name = 'FormDataField';
        newElement.value = 'No';
        $(sender).prepend(newElement);
    }

    //Set the value for user review - needs to happen before AppendFormDataFields
    if ($(sender).val().toLowerCase() == 'save request') {
        //Set to true if it exists as a hidden input
        $('#UserReview').val('Yes');
        //Set to checked if it exists as a checkbox
        $('#UserReview').prop("checked", true);
    }
    else {
        //Set to false if it exists as a hidden input
        $('#UserReview').val('No');
        //Set to unchecked if it exists as a checkbox
